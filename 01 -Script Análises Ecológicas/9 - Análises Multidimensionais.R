...
penguins_pad <- decostand(x = penguins_trait, method = "standard")
penguins_pad %>% 
  dplyr::summarise(across(where(is.numeric), 
                          ~var(.x, na.rm = TRUE)))
...
fviz_screeplot(pca.p, addlabels = TRUE, main = "", 
               xlab = "Dimensões",
...
env.pad <- decostand(x = env.sel, method = "standard")
...
env.bocaina <- decostand(bocaina.env[,-9], "standard")
...
## Mantel espacial
msr(compos_espac, listwORpolygons = lw, method = "pair")
