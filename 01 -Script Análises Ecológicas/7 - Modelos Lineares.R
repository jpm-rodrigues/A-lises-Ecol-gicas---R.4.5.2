...
t.test(Pareado$Riqueza[Pareado$Estado == "Pre-Queimada"], 
       Pareado$Riqueza[Pareado$Estado == "Pos-Queimada"], 
       paired = TRUE)
### Gráfico
ggpaired(Pareado, x = "Estado", y = "Riqueza",
         color = "Estado", line.color = "gray", line.size = 0.8, 
         palette = c("darkorange", "cyan4"), width = 0.5, 
         point.linewidth = 4, xlab = "Estado das localidades", 
...
check_model(modelo_regressao_mul)
...
### Teste de Tuckey's honest significant difference
pairs(emmeans(model_bloco, ~ Pocas), adjust = "tukey")
