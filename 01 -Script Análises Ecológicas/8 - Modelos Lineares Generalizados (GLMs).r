...
eosinofilo %>% 
  ...
  geom_errorbar(aes(ymin=eosinofilo-se, ymax=eosinofilo +se), 
                width=.1, linewidth = 1.1, position=pd) +
...
## Diagnose avançada
simulationBion <- simulateResiduals(fittedModel = mod1)
plot(simulationBion)

## Diagnose avançada
plot(binned_residuals(mod1))
...
## Diagnóse avançada
simulationBion <- simulateResiduals(fittedModel = mod_log)
plot(simulationBion)

## Diagnóse avançada
plot(binned_residuals(mod_log))
...
simulationOutput <- simulateResiduals(fittedModel = hur_NB)
plot(simulationOutput)

## Diagnoses Modelo zero-inflated Poisson
simulationOutput <- simulateResiduals(fittedModel = ziP_mod2)
plot(simulationOutput)

## Diagnoses Modelo zero-inflated negative binomial
simulationOutput <- simulateResiduals(fittedModel = ziNB_mod2)
plot(simulationOutput) 
